...
public class ReviewDbStorage implements ReviewStorage, ReviewEvaluationStorage {
    ...
    public ReviewDbStorage(JdbcTemplate jdbcTemplate, @Qualifier("reviewRowMapper") RowMapper<Review> rowMapper) {
    ...
    public Review addReview(Review review) {
        ...
        jdbcTemplate.update(connection -> {
            PreparedStatement stmt = connection.prepareStatement(ADD_REVIEW, new String[]{"id"});
    ...
    public Collection<Review> findReviewsByFilmId(Long filmId, int count) {
        return jdbcTemplate.query(FIND_REVIEWS_BY_FILM, rowMapper, filmId, count);
    }

    @Override
    public Collection<Review> findAllReviews(int count) {
        return jdbcTemplate.query(FIND_ALL_REVIEWS, rowMapper, count);
    }

    @Override
    public Optional<Review> findReviewById(Long reviewId) {
        return jdbcTemplate.query(FIND_REVIEW_BY_ID, rowMapper, reviewId)
                .stream()
                .findFirst();
    }
    ...
    public void putLike(Long reviewId, Long userId) {
        jdbcTemplate.update(PUT_EVALUATION, reviewId, userId, true);
    }

    @Override
    public void putDislike(Long reviewId, Long userId) {
        jdbcTemplate.update(PUT_EVALUATION, reviewId, userId, false);
    }

    @Override
    public boolean deleteLike(Long reviewId, Long userId) {
        int rows = jdbcTemplate.update(DELETE_LIKE, reviewId, userId);
        return rows > 0;
    }

    @Override
    public boolean deleteDislike(Long reviewId, Long userId) {
        int rows = jdbcTemplate.update(DELETE_DISLIKE, reviewId, userId);
        return rows > 0;
    }

    @Override
    public boolean deleteEvaluation(Long reviewId, Long userId) {
        int rows = jdbcTemplate.update(DELETE_EVALUATION, reviewId, userId);
        return rows > 0;
    }
}
